...
def validate_email_local_part(local: str, allow_smtputf8: bool = True, allow_empty_local: bool = False,
    ...
    if DOT_ATOM_TEXT_INTL.match(local):
        ...
        if not allow_smtputf8:
            ...
            if bad_chars:
                raise EmailSyntaxError("Internationalized characters before the @-sign are not supported: " + ", ".join(
                    sorted(bad_chars)) + ".")
    ...
    # restrictions on the placement of dots, as in dot-atom local parts.
        ...
        if bad_chars:
            raise EmailSyntaxError(
                "The email address contains invalid characters in quotes before the @-sign: " + ", ".join(
                    sorted(bad_chars)) + ".")
...
def validate_email_domain_literal(domain_literal):
    ...
    if re.match(r"^[0-9\.]+$", domain_literal):
        try:
            ...
            raise EmailSyntaxError(
                f"The address in brackets after the @-sign is not valid: It is not an IPv4 address ({e}) or is missing an address literal tag.") from e
